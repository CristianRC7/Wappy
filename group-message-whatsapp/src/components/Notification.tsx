...
const Notification: React.FC = () => {
  const { sending, current, index, total, finished, results, clear } = useNotification();
    ...
    <>
      ...
      {sending && (
        <div className="fixed bottom-6 right-6 z-[100] bg-blue-700 text-white px-6 py-4 rounded-xl shadow-lg flex flex-col gap-1 min-w-[260px] animate-fade-in">
          ...
          <span className="text-sm">{`Enviando a: `}<span className="font-mono">{current || '...'}</span></span>
      ...
      {finished && (
        ...
        >
          <span className="font-semibold text-lg">¡Envío finalizado!</span>
          <span className="text-sm">Enviados: <span className="font-bold">{successCount}</span> / {total}</span>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black bg-opacity-10 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full relative">
            <button
              ...
              title="Cerrar"
            >×</button>
            <h2 className="text-2xl font-bold mb-4 text-blue-700">Resumen de envío</h2>
                ...
                <tbody>
                  {results.map((r, i) => (
                    <tr key={r.telefono + i}>
                      ...
                      <td className={`py-1 px-3 text-sm font-semibold ${r.status === 'enviado' ? 'text-green-600' : 'text-red-600'}`}>{r.status === 'enviado' ? 'Enviado' : 'Error'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
